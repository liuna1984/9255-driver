...
extern IMU_Type IMU_Data_Sec[100];
...
void Data_Store(void)
{
	...
	TxFrameDefData *p;
		...
		for(i=0;i<100;i++)
		{
			p->GYRO_X = (float)((double)IMU_Data_Sec[i].Data.GYRO_X/ 131);
			p->GYRO_Y = (float)((double)IMU_Data_Sec[i].Data.GYRO_Y / 131);
			p->GYRO_Z = (float)((double)IMU_Data_Sec[i].Data.GYRO_Z / 131);
			p->ACCEL_X = (float)((double)IMU_Data_Sec[i].Data.ACCEL_X / 2048 * 9.8);
			p->ACCEL_Y = (float)((double)IMU_Data_Sec[i].Data.ACCEL_Y / 2048 * 9.8);
			p->ACCEL_Z = (float)((double)IMU_Data_Sec[i].Data.ACCEL_Z / 2048 * 9.8);	
			p->Reserved_1 = 0x00;
			p->Reserved_2 = 0x00;
			p->Reserved_3 = 0x00;
			//p->Temperature = (int16_t)((((double)(IMU_Data_Sec[i].Data.TEMP-0)/326.8)+25.0)*100);
			p->Temperature =(int16_t)((((double)(IMU_Data_Sec[i].Data.TEMP - 0) / 333.87) + 21.0) * 100);
			p->Sync = 0x00;
			p->Reserved_4 = 0x00;
			p++;
		}
