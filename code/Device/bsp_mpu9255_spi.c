...
//	  CS9255_LOW;
//		// 低通滤波频率，典型值0x07(3600HZ), 此寄存器绝定Internal_Sample_Rate==8K
//		spi1_txbuf[0] = CONFIG;
//		spi1_txbuf[1] = 0x07;
//	  HAL_SPI_Transmit(&hspi1, &spi1_txbuf[0], 2, 100);
//	  CS9255_HIGH;
...
//		spi1_txbuf[0] = I2C_MST_DELAY_CTRL;
//		spi1_txbuf[1] = 0x01;
//	  HAL_SPI_Transmit(&hspi1, &spi1_txbuf[0], 2, 100);
//		CS9255_HIGH;
//		HAL_Delay(10);
...
//		// 加速度计自检、测量范围及高通滤波，典型值：0x18(不自检，16g);
...
//		HAL_Delay(10);
    ...
    CS9255_HIGH;
	  ...
	  CS9255_LOW;
		// 陀螺仪采样率，典型值：0x07(1kHz) (SAMPLE_RATE= Internal_Sample_Rate / (1 + SMPLRT_DIV) )  
		spi1_txbuf[0] = SMPLRT_DIV;
		spi1_txbuf[1] = 0x07;															//SAMPLE_RATE_125HZ
	  HAL_SPI_Transmit(&hspi1, &spi1_txbuf[0], 2, 100);
		CS9255_HIGH;
		HAL_Delay(10);
    ...
    CS9255_HIGH;
	  ...
	  CS9255_LOW;
		// 加速度计高通滤波频率，典型值0x08(1.13KHZ)
		spi1_txbuf[0] = ACCEL_CONFIG_2;
		spi1_txbuf[1] = 0x04;									//A_DLPF_CFG4_20Hz
	  HAL_SPI_Transmit(&hspi1, &spi1_txbuf[0], 2, 100);
		CS9255_HIGH;
		HAL_Delay(10);
		...
		// INT Pin / Bypass Enable Configuration    
		spi1_txbuf[0] = INT_PIN_CFG;
		spi1_txbuf[1] = 0x02;									//turn on Bypass Mode 
	  HAL_SPI_Transmit(&hspi1, &spi1_txbuf[0], 2, 100);
		CS9255_HIGH;
		HAL_Delay(50);
